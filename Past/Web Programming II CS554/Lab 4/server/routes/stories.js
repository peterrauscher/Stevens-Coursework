...
const listLimitKey = "storieslistlimit";
const pagesKey = "storiespages";
  ...
  try {
    ...
    if (!pageExists) {
      ...
      if (!response)
        return res
          .status(404)
          .json({ error: `There are no stories on page ${pageNum}` });
      let responseData = {
      ...
      console.log(
        `Returned stories page ${pageNum} from API request and cached response`
      );
      return res.status(200).json(responseData);
    } else {
      // If the page WAS found in cache, return from there
      const cachedPage = await client.hGet(pagesKey, pageNum.toString());
      console.log(`Returned stories page ${pageNum} from cache`);
