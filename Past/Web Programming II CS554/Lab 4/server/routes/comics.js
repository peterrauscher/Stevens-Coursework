...
const listLimitKey = "comicslistlimit";
const pagesKey = "comicspages";
  ...
  try {
    ...
    if (!exists) {
      ...
      if (!response)
        return res
          .status(404)
          .json({ error: "Comic with id " + id + " not found" });
  ...
  try {
    ...
    if (!pageExists) {
      ...
      if (!response)
        return res
          .status(404)
          .json({ error: `There are no comics on page ${pageNum}` });
      let responseData = {
      ...
      console.log(
        `Returned comics page ${pageNum} from API request and cached response`
      );
      return res.status(200).json(responseData);
    } else {
      // If the page WAS found in cache, return from there
      const cachedPage = await client.hGet(pagesKey, pageNum.toString());
      console.log(`Returned comics page ${pageNum} from cache`);
